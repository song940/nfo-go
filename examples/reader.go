...
func readMovieNfo(filename string) (*nfo.Movie, error) {
	f, err := os.Open(filename)
	defer f.Close()
	if err != nil {
		return nil, fmt.Errorf("Error opening %s: %v\n", filename, err)
	}
	m, err := nfo.ReadMovieNfo(f)
	if err != nil {
...
func readAllMovies(dirname string) ([]*nfo.Movie, error) {
	movies := make([]*nfo.Movie, 0)
...
func readTVShowNfo(filename string) (*nfo.TVShow, error) {
	f, err := os.Open(filename)
	defer f.Close()
	if err != nil {
		return nil, fmt.Errorf("Error opening %s: %v\n", filename, err)
	}
	m, err := nfo.ReadTVShowNfo(f)
	if err != nil {
		return nil, fmt.Errorf("Error reading %s: %v\n", filename, err)
	}
	return m, nil
...
func readAllTVShows(dirname string) ([]*nfo.TVShow, error) {
	tvShows := make([]*nfo.TVShow, 0)
	err := filepath.Walk(dirname, func(path string, info os.FileInfo, err error) error {
...
func readEpisodeNfo(filename string) (*nfo.Episode, error) {
	f, err := os.Open(filename)
	defer f.Close()
	if err != nil {
		return nil, fmt.Errorf("Error opening %s: %v\n", filename, err)
	}
	m, err := nfo.ReadEpisodeNfo(f)
	if err != nil {
		return nil, fmt.Errorf("Error reading %s: %v\n", filename, err)
	}
	return m, nil
...
func readAllEpisodes(dirname string) ([]*nfo.Episode, error) {
	tvShows := make([]*nfo.Episode, 0)
	err := filepath.Walk(dirname, func(path string, info os.FileInfo, err error) error {
